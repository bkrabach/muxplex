...
| `input_allowed_sessions` | `[]` | Exact session names that may receive API terminal input. A session not on this list is a 403 even when `input_enabled` is true — this is how your own working panes stay un-typeable. Exact names only (no globs). **Local-file-only**: can ONLY be set by editing `settings.json` on disk — deliberately not settable via `PATCH /api/settings` and not federation-syncable. |
