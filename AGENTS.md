...
supports tmux 3.0. Keep the feature version-gated so an older tmux never parses
it. `capture-pane -e` preserves OSC 8 input even when a terminal feature does
not forward it, so it is not proof: the regression test must read URI+label
bytes from a real attached PTY and again after detach/re-attach redraw, using
only an explicitly owned `-L` socket and exact cleanup.
