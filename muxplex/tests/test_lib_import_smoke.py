...
The point of the `lib/tmuxkit` extraction is that a SECOND application can
...
directory (so ``tmuxkit`` resolves through the INSTALLED distribution, not
...
# the library yet (see lib/tmuxkit/__init__.py's "what does NOT live
# here"). Imports below are the shipped surface; extend this list as later
# stages land those pieces.
_SURFACE_PROGRAM = """
import tmuxkit
from tmuxkit.proc import run_tmux, set_env_factory, tmux_env
...
from tmuxkit.presence import (
    MANIFEST_SCHEMA_VERSION,
    compute_restore_plan,
    mark_restored,
    update_manifest,
)
from tmuxkit.bell import build_alert_bell_hook, poll_bell_flag
from tmuxkit.keys import ALLOWED_KEYS, MAX_KEYS, MAX_TEXT_BYTES
from tmuxkit.cgroup import should_escape, wrap_exec_argv, wrap_shell_argv
...
def _fresh_import_modules(tmp_path) -> set[str]:
    ...
    assert result.returncode == 0, (
        f"importing the tmuxkit public surface failed in a fresh "
        f"interpreter:\n{result.stderr}"
    )
...
def test_public_surface_imports_without_the_muxplex_server(tmp_path):
    ...
    assert not offenders, (
        f"importing tmuxkit's public surface dragged in server-side "
        f"modules: {offenders}. The library must stay importable by a "
        f"second app with ZERO muxplex/server weight (plan §13.1, §14.1) "
        f"-- this means the seam is wrong. Find which lib/tmuxkit module "
...
def test_lib_distribution_declares_zero_dependencies():
    ...
    deps = [
        r
        for r in (requires("tmuxkit") or [])
        # optional extras (e.g. the dev test extras) are not runtime deps
        if "extra ==" not in r
    ]
    assert deps == [], (
        f"tmuxkit declares runtime dependencies: {deps}. The library is "
